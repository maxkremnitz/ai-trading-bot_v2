...
class MeanReversionStrategy(BaseStrategy):
    ...
    TRADING_HOURS = {
        '^GDAXI': {
            'open': dt_time(7, 0),    # 9:00 MESZ = 7:00 UTC
            'close': dt_time(17, 30), # 19:30 MESZ = 17:30 UTC
            'weekdays': [0, 1, 2, 3, 4]
        },
        'GC=F': {
            'open': dt_time(1, 0),    # 20:00 ET Vorabend = 1:00 UTC
            'close': dt_time(23, 30), # 18:30 ET = 23:30 UTC
            'weekdays': [0, 1, 2, 3, 4]
        }
    }
    ...
    def calculate_technical_indicators(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        try:
            if len(data) < self.bb_period * 2:
                logger.warning(f"Nicht genug Daten für Bollinger Bands (nur {len(data)} Punkte)")
